...
import ScheduleReplacement from '@/models/api/entities/v2/base/ScheduleReplacement';
...
export default class ScheduleReplacementRepository implements IModernScheduleRepository<ScheduleReplacement> {
  ...
  public async getDataFromAPIForGroup(targetGroup: string, targetDate: Date, remainAttempts: number): Promise<ScheduleReplacement> {
  ...
  public async tryToGetDataFromAPIForGroup(targetGroup: string, targetDate: Date): Promise<ScheduleReplacement> {
  ...
  public async getDataFromAPIForTeacher(teacherId: number, targetDate: Date, remainAttempts: number): Promise<ScheduleReplacement> {
    let attempts = remainAttempts;
  ...
  public async tryToGetDataFromAPIForTeacher(teacherId: number, targetDate: Date): Promise<ScheduleReplacement> {
